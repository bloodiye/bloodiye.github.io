    ...
    ws.onopen = () => {
        ws.send(
          JSON.stringify({
            op: 2,
            d: {
              subscribe_to_id: "356743796513308673",
            },
          })
        );
    ...
    ws.onmessage = (msg) => {
     ...
     if (!['INIT_STATE', 'PRESENCE_UPDATE'].includes(msg.t)) return;
       let user = msg.d;
        ...
        document.getElementsByClassName("adisspp")[0].src = `https://cdn.discordapp.com/avatars/356743796513308673/${user.discord_user.avatar}?size=4096` 
        document.getElementsByClassName("adisspp2")[0].src = `https://cdn.discordapp.com/avatars/356743796513308673/${user.discord_user.avatar}?size=4096` 
       if (user.discord_status == "online") {
        ...
        async function customstatus() {
            const presence = (await fetch("https://api.lanyard.rest/v1/users/356743796513308673").then(_res => _res.json()).catch(() => null))?.data;
            if (!presence) return;
